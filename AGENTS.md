...
- **Longhorn backup S3 key** (`LONGHORN_BACKUP_SECRET_KEY`): Only present when
  ...
  Key auto-generated by `oci_identity_customer_secret_key`; it grants read/write on one
  named backup bucket. Risk accepted (opt-in feature, limited blast radius).
