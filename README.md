...
| <a name="input_vault_secret_id_gitops_ssh_key"></a> [vault\_secret\_id\_gitops\_ssh\_key](#input\_vault\_secret\_id\_gitops\_ssh\_key) | OCI Vault secret OCID containing the SSH private key used by ArgoCD to clone the gitops repo (e.g. Codeberg deploy key). Pre-create the secret manually in OCI Vault and paste the OCID here. Leave empty to skip ArgoCD repo secret creation (only suitable when gitops\_repo\_url is a public HTTPS repo). | `string` | `""` | no |
